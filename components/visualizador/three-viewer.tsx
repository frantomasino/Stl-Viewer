...
const ThreeViewer = forwardRef<ThreeViewerHandle, ThreeViewerProps>(
  (
    ...
    useEffect(() => {
      ...
      const gizmo = new ViewportGizmo(camera, renderer, {
        ...
        corners: {
          ...
          radius: 1,
          smoothness: 18,
          hover: { color: 0xffffff, opacity: 1, scale: 0.2 },
        },
        edges: {
          enabled: false,
          color: 0xf2f2c2,
      ...
      const onCanvasClick = (event: MouseEvent) => {
        ...
        if (intersects.length > 0) {
          ...
          guiParamsRef.current.color =
            "#" + (mat?.color?.getHexString?.() || "cccccc");
      ...
      return () => {
        ...
        if (renderer.domElement.parentNode)
          (renderer.domElement.parentNode as HTMLElement).removeChild(
            renderer.domElement
          );
        scene.clear();
    ...
    useEffect(() => {
      ...
      const onModelReady = (root: THREE.Object3D) => {
        ...
        setControlsState((p) => ({
          ...
          selectedMesh: 0,
          color: `#${
            getMat(selectableMeshesRef.current[0])?.color.getHexString?.() ||
            "cccccc"
          }`,
    ...
    function forEachMat(
      ...
      const m = mesh.material as any;
      if (Array.isArray(m))
        m.forEach((mm: any) => mm?.isMeshStandardMaterial && fn(mm));
    ...
    function updateExplode() {
      meshOriginalPositionsRef.current.forEach(
        ({ mesh, original, direction }) => {
          mesh.position
            .copy(original)
            .add(direction.clone().multiplyScalar(explodeFactorRef.current));
        }
      );
    ...
    function updateClippingState() {
      ...
      selectableMeshesRef.current.forEach((mesh, i) => {
        forEachMat(mesh, (mat) => {
          mat.clippingPlanes =
            guiParamsRef.current.clipSelectedOnly &&
            i !== guiParamsRef.current.selectedMesh
              ? []
              : planes;
          mat.needsUpdate = true;
          mat.side = THREE.DoubleSide;
    ...
    function applyControls(u: Partial<ControlsState>) {
      ...
      if (u.selectedMesh !== undefined) {
        const i = Math.max(
          0,
          Math.min(u.selectedMesh, selectableMeshesRef.current.length - 1)
        );
      ...
      if (u.color !== undefined) {
        const mesh =
          selectableMeshesRef.current[guiParamsRef.current.selectedMesh];
        if (mesh) {
          forEachMat(mesh, (mat) => {
            try {
              mat.color.set(u.color!);
            } catch {}
            mat.needsUpdate = true;
            mat.side = THREE.DoubleSide;
          });
      ...
      if ((u.windowX || u.windowY || u.windowZ) && !isExploding())
        updateClippingState();
    ...
    function setupGUI() {
      ...
      gui
        ...
        .onChange(() => {
          setControlsState((p) => ({
            ...p,
            clipSelectedOnly: guiParamsRef.current.clipSelectedOnly,
          }));
          if (isExploding()) return;
          updateClippingState();
        });
