...
useEffect(() => {
  ...
  const filteredUsers = useMemo(() => {
    ...
    return usersList.filter((u) => [u.name, u.email, u.role].some((f) => normalize(f).includes(q)));
...
useEffect(() => {
  ...
  const handleProjectToggle = async (projectId: string, granted: boolean) => {
    ...
    setCountsByUid((prev) => ({ ...prev, [uid]: Math.max(0, prevCount + (granted ? 1 : -1)) }));
    ...
    toast({ title: "Access Updated", description: `${granted ? "Granted" : "Revoked"} access to ${projName}` });
  };
        ...
        <CardHeader className="space-y-3">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
            <div className="min-w-0">
              ...
              <CardDescription>Pick a user to manage project access</CardDescription>
            </div>
            <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
              <Button onClick={doRefresh} variant="outline" size="sm" disabled={refreshing} className="shrink-0" title="Refresh users & projects">
          ...
          {filteredUsers.length === 0 ? (
            ...
            filteredUsers.map((user) => {
              ...
              return (
                <div
                  key={user.id}
                  className={`p-3 border rounded-lg cursor-pointer transition-colors ${selectedUser?.id === user.id ? "bg-primary/10 border-primary" : "hover:bg-muted/50"}`}
  ...
  <CardTitle className="mb-2 sm:mb-0">
    {selectedUser ? `Projects for ${selectedUser.name}` : "Select a user"}
  </CardTitle>
    ...
    <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
      ...
      >
        ...
        <CardContent className="space-y-3 max-h-96 overflow-y-auto">
          {!selectedUser ? (
            <p className="text-center text-muted-foreground py-8">Select a user from the left to manage their project permissions.</p>
          ) : projList.length === 0 ? (
              ...
              return (
                <div key={project.id} className="flex items-center space-x-3 p-3 border rounded-lg">
                  ...
                  <div className="flex-1 min-w-0">
                    <label htmlFor={`project-${project.id}`} className="font-medium cursor-pointer truncate">
                      {project.name}
                    </label>
                    <div className="text-sm text-muted-foreground">
                      {project.type && <span className="mr-3">Type: {project.type}</span>}
                      {project.created && <span>Date: {(() => {
                        const c = project.created as any;
                        if (c && typeof c === "object" && typeof (c as any).toDate === "function") return (c as any).toDate().toLocaleDateString("es-AR");
                        const d = new Date(c);
